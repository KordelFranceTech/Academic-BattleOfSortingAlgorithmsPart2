...
from Lab2.quicksort import quicksort_driver, reset_counts_quicksort
from Lab2.quicksort_randomized import quicksort_randomized_driver, reset_counts_quicksort_randomized
from Lab2.quicksort_mot import quicksort_mot_driver, reset_counts_quicksort_mot
from Lab2.heap_sort import heap_sort, reset_counts_heap_sort
from Lab2.merge_sort_natural import merge_sort_natural, reset_counts_merge_sort_natural
from Lab2.merge_sort_2way import merge_sort_two_way, reset_counts_merge_sort_two_way
from Lab2.merge_sort_3way import merge_sort_three_way, reset_counts_merge_sort_three_way
from Lab2.constants import file_sizes, file_types, sort_algos
from Lab2.file_manager import generate_sorted_file, generate_reverse_sorted_file, generate_random_sorted_file
from Lab2 import graph_data as graph
...
time.sleep(2.0)
print('*****************************************************************************************')
print('\t\tStarting Quicksort Complexity Analysis Program\n')
print('_________________________________________________________________________________________')
time.sleep(2.0)
...
for size in file_sizes:                                                             # iterate through file sizes (n)
    for file_type in file_types:                                                    # iterate through file types
        if file_type == 'sorted':                                                   # check for sorted file type
            ...
            algo = 'qs'                                                            # perform quicksort
            predata_qs = [int] * len(predata)
            for i in range(0, len(predata)):
                predata_qs[i] = predata[i]
            postdata_qs, comps, exs, dt = quicksort_driver(predata_qs)
            metric_qs = m(n=int(size),
                              sort=str(file_type),
                              algo=str(algo),
                              comps=comps,
                              exs=exs,
                              predata=predata,
                              postdata=postdata_qs,
                              comp_eq='',
                              ex_eq='',
                              time=str(dt))
            data_metrics.append(metric_qs)
            reset_counts_quicksort()

            algo = 'qsr'                                                            # perform randomized quicksort
            predata_qsr = [int] * len(predata)
            for i in range(0, len(predata)):
                predata_qsr[i] = predata[i]
            postdata_qsr, comps, exs, dt = quicksort_randomized_driver(predata_qsr)
            metric_qsr = m(n=int(size),
                              sort=str(file_type),
                              algo=str(algo),
                              comps=comps,
                              exs=exs,
                              predata=predata,
                              postdata=postdata_qsr,
                              comp_eq='',
                              ex_eq='',
                              time=str(dt))
            data_metrics.append(metric_qsr)
            reset_counts_quicksort_randomized()

            algo = 'qsmot'                                                            # perform medium-of-three quicksort
            predata_qsmot = [int] * len(predata)
            for i in range(0, len(predata)):
                predata_qsmot[i] = predata[i]
            postdata_qsmot, comps, exs, dt = quicksort_mot_driver(predata_qsmot)
            metric_qsmot = m(n=int(size),
                              sort=str(file_type),
                              algo=str(algo),
                              comps=comps,
                              exs=exs,
                              predata=predata,
                              postdata=postdata_qsmot,
                              comp_eq='',
                              ex_eq='',
                              time=str(dt))
            data_metrics.append(metric_qsmot)
            reset_counts_quicksort_mot()
...
print(f'\nA copy of the table above along with individual .csv files for each of the '
      f'{len(file_types) * len(file_sizes) * len(sort_algos)} trace runs may be found in the `output_files` folder.')
