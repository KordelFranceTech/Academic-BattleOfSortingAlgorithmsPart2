# quicksort.py
# Kordel France
########################################################################################################################
# This file provides functions to sort an array of integers in ascending order using a two-way merge sort.
########################################################################################################################
import time

QSMOT_COMP = 0
QSMOT_EX = 0

def partition_array_mot(qArray, start_index, end_index):
    """
    Determines the median point of partitioning before regular partitioning for the next epoch of quicksorting.
    :param qArray: the array to partition from.
    :param start_index: the index to start the array sort at
    :param end_index: the index to end the array sort at
    :return pivot_index: the specific index to pivot for the next partition..
    """
    global QSMOT_COMP
    global QSMOT_EX
    i_index = start_index
    j_index = end_index
    k_index = int((j_index - i_index) / 2)
    pivot_index = k_index
    if k_index < i_index and i_index < j_index:
        pivot_index = i_index
        QSMOT_COMP += 1
    elif j_index < i_index and i_index < k_index:
        pivot_index = i_index
        QSMOT_COMP += 1
    elif i_index < j_index and j_index < k_index:
        pivot_index = j_index
        QSMOT_COMP += 1
    elif k_index < j_index and j_index < i_index:
        pivot_index = j_index
        QSMOT_COMP += 1
    elif i_index < k_index and k_index < j_index:
        pivot_index = k_index
        QSMOT_COMP += 1


    qArray[start_index], qArray[pivot_index] = qArray[pivot_index], qArray[start_index]
    QSMOT_EX += 1

    return partition_array(qArray, start_index, end_index)


def partition_array(qArray, start_index, end_index):
    """
    Determines which point to partition the array at for the next epoch of quicksorting.
    :param qArray: the array to partition from.
    :param start_index: the index to start the array sort at
    :param end_index: the index to end the array sort at
    :return pivot_index: the specific index to pivot for the next partition..
    """
    global QSMOT_COMP
    global QSMOT_EX
    pivot_index = start_index
    pivot_point = start_index + 1

    for i in range(start_index + 1, end_index + 1):
        if qArray[i] <= qArray[pivot_index]:
            QSMOT_COMP += 1
            qArray[pivot_point], qArray[i] = qArray[i], qArray[pivot_point]
            QSMOT_EX += 1
            pivot_point += 1
    qArray[pivot_index], qArray[pivot_point - 1] = qArray[pivot_point - 1], qArray[pivot_index]
    QSMOT_EX += 1
    pivot_index = pivot_point - 1
    return pivot_index


def quicksort_mot(qArray, start_index, end_index):
    """
    Actually implements this style of the quicksort algorithm.
    It is the main routine called during recursion.
    :param qArray: the array to partition from.
    :param start_index: the index to start the array sort at
    :param end_index: the index to end the array sort at
    :return qArray: the sorted array.
    """
    if (start_index < end_index):
        partition_index = partition_array_mot(qArray, start_index, end_index)
        quicksort_mot(qArray, start_index, partition_index - 1)
        quicksort_mot(qArray, partition_index + 1, end_index)
    # print(qArray)
    return qArray


def quicksort_mot_driver(data_arr):
    """
    Driver for implementing median-of-three version of quicksort.
    :param data_arr: the array to split and merge sort from.
    ;return data_arr: the sorted array.
    ;return QSMOT_COMP: number of comparisons performed
    ;return QSMOT_EX: number of exchanges performed
    ;return time: The time it takes for the algorithm to sort the file.
    """
    start_time = time.time()
    global QSMOT_COMP
    global QSMOT_EX

    qArray_sorted = quicksort_mot(data_arr, 0, len(data_arr) - 1)

    end_time = time.time()
    delta_time = end_time - start_time
    return qArray_sorted, QSMOT_COMP, QSMOT_EX, "{:.6f}".format(delta_time)


def reset_counts_quicksort_mot():
    """
    Resets the comparisons and exchanges counters so they are initialized for a new sort.
    """
    # reset comparison and exchange counts for next run
    global QSMOT_COMP
    global QSMOT_EX
    QSMOT_COMP = 0
    QSMOT_EX = 0


# q = [3, 3, 4, 4, 5, 6, 7, 45, 32, 76, 54, 65]
# quicksort_mot(q, 0, len(q) - 1)

