...
import csv
from Lab2.Metric import Metric as m
from Lab2.constants import file_sizes, file_types, sort_algos
...
def stratify_data_algos(sorted_metrics, rev_sorted_metrics, random_metrics):
	...
	graph_exchanges('Sorted', qs_data, qsr_data, qsmot_data,  m1x_data, m2x_data, m3x_data, heap_data)
	...
	graph_exchanges('Reverse Sorted', qs_data, qsr_data, qsmot_data, m1x_data, m2x_data, m3x_data, heap_data)
	# re-init the data for next sorting category
	qs_data.clear()
	qsr_data.clear()
	qsmot_data.clear()
	...
	graph_exchanges('Randomized', qs_data, qsr_data, qsmot_data, m1x_data, m2x_data, m3x_data, heap_data)


def graph_exchanges(title, qs_data, qsr_data, qsmot_data, m1x_data, m2x_data, m3x_data, heap_data):
	"""
	...
	# iterate through all quicksort objects to extract # comparisons and # exchanges for graphing
	for metric in qs_data:
		met = metric
		m0 = m(met.n, met.sort, met.algo, met.comps, met.exs, met.predata, met.postdata, '', '', met.time)
		qs_x_vals.append(m0.n)
		qs_ex_vals.append(m0.exs)
		qs_comp_vals.append(m0.comps)

	# iterate through all quicksort randomized objects to extract # comparisons and # exchanges for graphing
	for metric in qsr_data:
		met = metric
		m0 = m(met.n, met.sort, met.algo, met.comps, met.exs, met.predata, met.postdata, '', '', met.time)
		qsr_x_vals.append(m0.n)
		qsr_ex_vals.append(m0.exs)
		qsr_comp_vals.append(m0.comps)

	# iterate through all quicksort median-of-three objects to extract # comparisons and # exchanges for graphing
	for metric in qsmot_data:
		met = metric
		m0 = m(met.n, met.sort, met.algo, met.comps, met.exs, met.predata, met.postdata, '', '', met.time)
		qsmot_x_vals.append(m0.n)
		qsmot_ex_vals.append(m0.exs)
		qsmot_comp_vals.append(m0.comps)
	...
	# iterate through all quicksort objects to compute the fitted exponential regression curves for trajectory
	#		of # comparisons and # exchanges as n scales larger.
	# also write to file
	for qs in qs_data:
		qs.comp_eq = qs.fitPowerRegressionCurveComparisons(qs_x_vals, qs_comp_vals)
		qs.ex_eq = qs.fitPowerRegressionCurveExchanges(qs_x_vals, qs_ex_vals)
		write_data_to_file(qs)
		final_metrics.append(qs)

	# iterate through all quicksort randomized objects to compute the fitted exponential regression curves for trajectory
	#		of # comparisons and # exchanges as n scales larger.
	# also write to file
	for qsr in qsr_data:
		qsr.comp_eq = qsr.fitPowerRegressionCurveComparisons(qsr_x_vals, qsr_comp_vals)
		qsr.ex_eq = qsr.fitPowerRegressionCurveExchanges(qsr_x_vals, qsr_ex_vals)
		write_data_to_file(qsr)
		final_metrics.append(qsr)

	# iterate through all quicksort median-of-three objects to compute the fitted exponential regression curves for trajectory
	#		of # comparisons and # exchanges as n scales larger.
	# also write to file
	for qsmot in qsmot_data:
		qsmot.comp_eq = qsmot.fitPowerRegressionCurveComparisons(qsmot_x_vals, qsmot_comp_vals)
		qsmot.ex_eq = qsmot.fitPowerRegressionCurveExchanges(qsmot_x_vals, qsmot_ex_vals)
		write_data_to_file(qsmot)
		final_metrics.append(qsmot)
	...
	plt.scatter(qs_x_vals, qs_ex_vals, color='black')
	plt.scatter(qsr_x_vals, qsr_ex_vals, color='black')
	plt.scatter(qsmot_x_vals, qsmot_ex_vals, color='black')
	plt.scatter(m1x_x_vals, m1x_ex_vals, color='black')
	plt.scatter(m2x_x_vals, m2x_ex_vals, color='black')
	plt.scatter(m3x_x_vals, m3x_ex_vals, color='black')
	plt.scatter(heap_x_vals, heap_ex_vals, color='black')
	plt.scatter(qs_x_vals, qs_comp_vals, color='black')
	plt.scatter(qsr_x_vals, qsr_comp_vals, color='black')
	plt.scatter(qsmot_x_vals, qsmot_comp_vals, color='black')
	plt.scatter(m1x_x_vals, m1x_comp_vals, color='black')
	plt.scatter(m2x_x_vals, m2x_comp_vals, color='black')
	plt.scatter(m3x_x_vals, m3x_comp_vals, color='black')
	plt.scatter(heap_x_vals, heap_comp_vals, color='black')

	# overlay line graphs of # comparisons per algorithm
	plt.plot(qs_x_vals, qs_comp_vals, label='(comparisons)  quicksort sort', color='red')
	plt.plot(qsr_x_vals, qsr_comp_vals, label='(comparisons)  quicksort rand. sort', color='blue')
	plt.plot(qsmot_x_vals, qsmot_comp_vals, label='(comparisons)  quicksort MOT sort', color='purple')
	plt.plot(m1x_x_vals, m1x_comp_vals, label='(comparisons)  natural merge sort', color='yellow')
	plt.plot(m2x_x_vals, m2x_comp_vals, label='(comparisons)  2-way merge sort', color='black')
	plt.plot(m3x_x_vals, m3x_comp_vals, label='(comparisons)  3-way merge sort', color='grey')
	plt.plot(heap_x_vals, heap_comp_vals, label='(comparisons)  heap sort', color='green')

	# overlay dashed line graphs of # exchanges per algorithm
	plt.plot(qs_x_vals, qs_ex_vals, label='(exchanges)     quicksort', linestyle='--', color='red')
	plt.plot(qsr_x_vals, qsr_ex_vals, label='(exchanges)     quicksort rand.', linestyle='--', color='blue')
	plt.plot(qsmot_x_vals, qsmot_ex_vals, label='(exchanges)     quicksort MOT', linestyle='--', color='purple')
	plt.plot(m1x_x_vals, m1x_ex_vals, label='(exchanges)     natural merge sort', linestyle='--', color='yellow')
	plt.plot(m2x_x_vals, m2x_ex_vals, label='(exchanges)     2-way merge sort', linestyle='--', color='black')
	plt.plot(m3x_x_vals, m3x_ex_vals, label='(exchanges)     3-way merge sort', linestyle='--', color='grey')
	plt.plot(heap_x_vals, heap_ex_vals, label='(exchanges)     heap sort', linestyle='--', color='green')

	# format the graph and show it to the user
	plot_ylim = max(max(qs_ex_vals),
					max(qsr_ex_vals),
					max(qsmot_ex_vals),
					max(m1x_ex_vals),
					max(m2x_x_vals),
	...
	plt.ylabel('number of exchanges & comparisons')
	plt.xlim(0, max(file_sizes) * 1.05)
	plt.ylim(0, plot_ylim * 1.05)
	plt.legend()
...
def write_data_to_file(data):
	...
	if metric.algo == 'qs' or metric.algo == 'quicksort':
		metric.algo = 'quicksort'
	elif metric.algo == 'qsr' or metric.algo == 'quicksort_rand':
		metric.algo = 'quicksort_rand'
	elif metric.algo == 'qsmot' or metric.algo == 'quicksort_mot':
		metric.algo = 'quicksort_mot'
	elif metric.algo == 'm1x':
	...
	elif metric.algo == 'heap_sort' or metric.algo == 'heap':
